...
class SQLAlchemyTableModel(QAbstractTableModel):
    ...
    def add_task(self, name, status):
        task = Task(name=name, status=status, progress=0)
    ...
    def move_up(self, row):
        if row <= 0:
            return
        self.tasks[row - 1].id, self.tasks[row].id = self.tasks[row].id, self.tasks[row - 1].id
        self.session.commit()
        self.refresh_data()
        self.layoutChanged.emit()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SQLAlchemy 表格 + 按钮 + 进度条")
        self.session = Session()
    ...
    def add_task(self):
        name = f"任务{len(self.model.tasks) + 1}"
        self.model.add_task(name, "未完成")
